...
//
// Рассмотрим пример. Есть массив
// 3 4 1
// 2 2 5
// 2 4 2
// 2 2 1
//
// Совершаем клик по второму столбцу
// 2 2 5
// 2 2 1
// 3 4 1
// 2 4 2
...
// class Result
// {
// 	public static string Execute(List<List<int>> datas, List<int> sort)
// 	{
// 		for (int i = 0; i < sort.Count; i++)
// 		{
// 			datas = datas.OrderBy(p => p[sort[i] - 1]).ToList();
// 		}
//
// 		string result = null;
//
...
// 		{
// 			result += string.Join(" ", datas[i]) + Environment.NewLine;
// 		}
// 		
// 		return result;
// 	}
...
// 			var count = Console.ReadLine().TrimEnd().Split(' ').ToList().Select(arTemp => Convert.ToInt32(arTemp)).ToList();
//
// 			var datas = new List<List<int>>();
// 			for (int j = 0; j < count.FirstOrDefault(); j++)
// 			{
// 				var data = Console.ReadLine().TrimEnd().Split(' ').ToList().Select(arTemp => Convert.ToInt32(arTemp)).ToList();
// 				datas.Add(data);
// 			}
//
// 			
// 			var countSort = Console.ReadLine();
// 			var sort = Console.ReadLine().TrimEnd().Split(' ').ToList().Select(arTemp => Convert.ToInt32(arTemp)).ToList();
// 			
// 			string result = Result.Execute(datas, sort);
// 			results.Add(result);
// 			
// 			
// 		}
